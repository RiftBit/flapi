...
@jsonrpc.method('Index.index', authenticated=auth.static_auth)
def index():
    return u'Welcome to FLAPI'


@jsonrpc.method('Index.echo(name=str) -> str', validate=True, authenticated=auth.static_auth)
def echo(name='Flask JSON-RPC'):
    return u'Hello {0}'.format(name)


@jsonrpc.method('Index.hello', authenticated=auth.static_auth)
def hello(name):
    return u'Hello {0}'.format(name)


@jsonrpc.method('Index.helloDefaultArgs', authenticated=auth.static_auth)
